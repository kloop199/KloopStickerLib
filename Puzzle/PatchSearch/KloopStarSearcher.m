...
#import "KloopPriorityQueue.h"

@implementation KloopStarSearcher
...
- (NSMutableArray *)search {
    ...
    [(id<KloopStarSearcherStatus>)[self startStatus] setGValue:0];
    
    // 关闭堆，存放已搜索过的状态
    NSMutableDictionary *close = [NSMutableDictionary dictionary];
    // 开放队列，存放由已搜索过的状态所扩展出来的未搜索状态
    // 使用优先队列
    KloopPriorityQueue *open = [KloopPriorityQueue queueWithComparator:^NSComparisonResult(id<KloopStarSearcherStatus> obj1, id<KloopStarSearcherStatus> obj2) {
    ...
    while (open.count > 0) {
        ...
        // 对各个子状进行代价估算
        [childStatus enumerateObjectsUsingBlock:^(id<KloopStarSearcherStatus>   obj, NSUInteger idx, BOOL * _Nonnull stop) {
            // 子状态的实际代价比本状态大1
