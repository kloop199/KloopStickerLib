...
//    }
    ...
    KloopPiecesStatus *status = [[KloopPiecesStatus alloc] init];
    ...
    for (int i=0; i<rows; i++) {
        for (int j=0; j<cols; j++) {
            ...
            KloopSquareModel *model;
            if ((i==rows-1) && (j== cols-1)) {
                model = [KloopSquareModel squuareWithID:i*cols+j image: nil];
            } else {
                model = [KloopSquareModel squuareWithID:i*cols+j image: [image cutImageInRect: rect]];
            }
...
- (BOOL)equalWithStatus:(KloopPiecesStatus *)status {
...
//    [self.pieceArray enumerateObjectsUsingBlock:^(KloopSquareModel * obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setParentStatus:(id<KloopPathSearcherStatus>)parentStatus {
    _parentStatus = parentStatus;
}

- (id<KloopPathSearcherStatus>)parentStatus {
...
- (void)addChildStatusIndex:(NSInteger)index toArray:(NSMutableArray *)array {
    // 排除父状态
    if ([self parentStatus] && [(KloopPiecesStatus *)[self parentStatus] indexOfWhite] == index) {
        return;
    }
    if (![self canMoveToIndex:index]) {
        return;
    }
    KloopPiecesStatus *status = [self copyStatus];
...
/// 估算从当前状态到目标状态的代价
- (NSInteger)estimateToTargetStatus:(id<KloopPathSearcherStatus>)targetStatus {
    KloopPiecesStatus *target = (KloopPiecesStatus *)targetStatus;
...
//        KloopSquareModel *currentPiece = self.pieceArray[index];
//        KloopSquareModel *targetPiece = target.pieceArray[index];
...
-(BOOL)canRecovery:(NSMutableArray *)randomArr {
    ...
    for (int i = 0; i < randomArr.count-1; i++) {
        ...
        for (int j = i + 1; j < randomArr.count; j++) {
            KloopSquareModel *modeli = randomArr[i];
            KloopSquareModel *modelj = randomArr[j];
...
- (NSMutableArray *)currentIndexs {
    ...
    [self.pieceArrayModel enumerateObjectsUsingBlock:^(KloopSquareModel *  obj, NSUInteger idx, BOOL * _Nonnull stop) {
